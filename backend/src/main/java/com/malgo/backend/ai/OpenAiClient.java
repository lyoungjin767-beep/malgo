...
public class OpenAiClient {
    ...
    public String chat(
        ...
        String prompt = """
                ...
                상대방 말투/스타일은 답변의 실제 어투를 정하는 값이다. 스타일을 설명만 하거나
                표준어로 답변하지 말고, 제목을 제외한 의미·맥락 설명·사용 예시·추천 표현의
                모든 자연어 문장에 선택된 말투 하나를 처음부터 끝까지 일관되게 적용한다.
                한국어 설명도 반드시 선택한 말투로 작성하고, 외국어 예문과 추천 표현도
                같은 말투의 의도에 맞는 현지 표현을 선택한다.
                ...
                - DIALECT: 사투리. 한국어 설명과 모든 한국어 자연어 문장은 실제 한국어 사투리 어조로
                  작성한다. 외국어 사용 예시와 추천 표현에도 targetCountry와 targetLanguage에서 실제로
                  쓰이는 지역적 말투 또는 구어적 특징을 반영한다.
                - DIALECT인 경우 특정 지역이 지정되지 않았다면, 사용자가 읽고 이해할 수 있는 자연스러운
                  한국어 사투리를 일관되게 사용한다. 표준어 답변으로 바꾸지 않는다.
                - DIALECT인 경우 업무·첫 만남 등 격식이 필요한 맥락에서도 선택된 사투리 어조를 유지하되,
                  내용 자체는 예의 있고 상황에 맞게 작성한다.
